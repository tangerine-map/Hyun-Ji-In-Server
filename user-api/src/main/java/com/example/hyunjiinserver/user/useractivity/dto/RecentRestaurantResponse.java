...
import com.example.hyunjiinserver.core.useractivity.application.RecentRestaurantResult;
import io.swagger.v3.oas.annotations.media.Schema;
import java.time.OffsetDateTime;

public record RecentRestaurantResponse(
        @Schema(description = "식당 ID입니다. 선택 시 식당 상세로 이동합니다.", example = "1")
        Long restaurantId,
        ...
        @Schema(description = "마지막으로 열람한 시각입니다.", example = "2026-07-22T12:30:00+09:00")
        OffsetDateTime viewedAt
) {

    public static RecentRestaurantResponse from(RecentRestaurantResult result) {
        return new RecentRestaurantResponse(
                result.restaurantId(),
                result.name(),
